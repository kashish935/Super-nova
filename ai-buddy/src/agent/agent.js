...
//config - additional metadata to the state graph while invokoing the agent
const graph = new StateGraph(MessagesAnnotation)
    .addNode("tools", async (state, config) => {
        ...
        const toolCallResults = await Promise.all(toolsCall.map(async (call) => {
            ...
            const toolResult = await tool.func({ ...toolInput, token: config.metadata.token })//no need to invoke it we do directly the function call here 
